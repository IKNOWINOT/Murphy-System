...
class EnhancedGroqLLMProvider:
    ...
    def generate_with_aristotle(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            # Try to find and evaluate math expression
            # Remove common words and extract numbers/operators
            math_expr = prompt
            for word in ['what is', 'calculate', 'equals', '=', '?']:
            ...
            # Replace 'divided by' with '/'
            math_expr = math_expr.replace('divided by', '/').replace('all divided by', '/')
            
            try:
                # Safely evaluate the expression
                result = eval(math_expr, {"__builtins__": {}}, {})
                response = f"The answer is: {result}"
                logger.info(f"Aristotle calculated: {math_expr} = {result}")
            except:
                # If eval fails, use Groq as fallback
                response = self.generate_with_groq(f"Calculate this math problem and show your work: {prompt}")
