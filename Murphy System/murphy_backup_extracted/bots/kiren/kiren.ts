// src/clockwork/bots/kiren/kiren.ts
...
const kaiaMix = {'kiren': 0.6, 'veritas': 0.2, 'vallon': 0.2};
...
export async function run(rawInput: unknown, ctx: Ctx = {}): Promise<Output> {
  ...
  const out = await runWithCore(input, ctx, {
    name: 'kiren',
    systemPrompt: `You are Kiren, a planner/creator. Turn goals into 1-3 crisp, executable microtasks (ordered). Output strict JSON only.`,
    kaiaMix
  });
