// src/clockwork/bots/cad_bot/rollcall.ts
import { z } from 'zod';
...
const RollcallInputSchema = z.object({
  ...
  attachments: z.array(z.object({ type: z.string(), url: z.string().optional(), text: z.string().optional() })).optional(),
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['cad','step','stp','iges','igs','stl','dxf','dwg','drawing','bom','tolerance','gd&t','dimension','units','mesh','repair','assembly','explode','render','export','entities','units'];
  let score = 0; for (const k of keywords) if (task.includes(k)) score += 0.14;
  if (input.attachments?.length) score += 0.12;
  if (task.split(/\s+/).length <= 14) score += 0.06;
  const confidence = Math.min(1, Math.max(0, score));

  const est_cost_usd = 0.0025;
  const must_have_inputs: string[] = [];
  if (task.includes('export') || task.includes('dxf') || task.includes('step') || task.includes('stp')) must_have_inputs.push('cad_engine','file_access');
  if (task.includes('bom')) must_have_inputs.push('cad_engine');
