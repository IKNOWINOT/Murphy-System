...
SOC2_CONTROLS: List[ComplianceControl] = [
    ComplianceControl(
        ...
        murphy_component="Murphy Gate — EXECUTIVE/COMPLIANCE gates",
        status="PARTIAL",
        gap="Gate audit logs not yet persisted to immutable store.",
        remediation="Integrate gate events with append-only audit log (e.g., AWS CloudTrail).",
        due_date="2026-06-01",
    ),
    ComplianceControl(
        framework="SOC 2 Type II",
        ...
        murphy_component="ConfidenceEngine — real-time scoring",
        status="IMPLEMENTED",
    ),
    ComplianceControl(
        framework="SOC 2 Type II",
        ...
        murphy_component="GateCompiler — dynamic gate synthesis",
        status="PARTIAL",
        gap="Gate compilation changes are not gated behind change-management workflow.",
        remediation="Require PR review for changes to gate rule tables.",
        due_date="2026-05-15",
    ),
    ComplianceControl(
        framework="SOC 2 Type II",
        ...
        murphy_component="Murphy System orchestrator",
        status="PLANNED",
        gap="No SLO dashboards deployed.",
        remediation="Deploy Prometheus + Grafana SLO dashboards.",
        due_date="2026-07-01",
    ),
    ComplianceControl(
        framework="SOC 2 Type II",
...
ISO27001_CONTROLS: List[ComplianceControl] = [
    ComplianceControl(
        ...
        murphy_component="Murphy Gate — HITL approval workflow",
        status="PARTIAL",
        gap="Role-based gate bypass not yet enforced at API layer.",
        remediation="Implement RBAC middleware before gate evaluation endpoints.",
        due_date="2026-06-15",
    ),
    ComplianceControl(
        framework="ISO 27001",
        ...
        murphy_component="ConfidenceResult — rationale field",
        status="PARTIAL",
        gap="Rationale strings not shipped to SIEM.",
        remediation="Forward ConfidenceResult events to SIEM via structured log pipeline.",
        due_date="2026-06-01",
    ),
    ComplianceControl(
        framework="ISO 27001",
    ...
    ComplianceControl(
        ...
        murphy_component="Murphy Gate — COMPLIANCE gate",
        status="PLANNED",
        gap="PII detection in AI output not yet integrated with COMPLIANCE gate.",
        remediation="Add PII scanner (e.g., Presidio) as pre-gate hook.",
        due_date="2026-07-15",
    ),
]
...
HIPAA_CONTROLS: List[ComplianceControl] = [
    ComplianceControl(
        ...
        murphy_component="Murphy Gate — COMPLIANCE + HITL gates",
        status="PARTIAL",
        gap="ePHI classification not surfaced in confidence hazard score.",
        remediation="Train domain model to flag ePHI presence in prompt, raise H(x).",
        due_date="2026-06-01",
    ),
    ComplianceControl(
        framework="HIPAA",
        ...
        murphy_component="ConfidenceResult — timestamp + rationale",
        status="PARTIAL",
        gap="Audit records not stored in HIPAA-compliant backend.",
        remediation="Route ConfidenceResult logs to encrypted, access-controlled audit store.",
        due_date="2026-05-30",
    ),
    ComplianceControl(
        framework="HIPAA",
        ...
        murphy_component="Murphy System API layer",
        status="IMPLEMENTED",
    ),
    ComplianceControl(
        framework="HIPAA",
        ...
        murphy_component="BLOCK_EXECUTION gate action",
        status="IMPLEMENTED",
    ),
]
