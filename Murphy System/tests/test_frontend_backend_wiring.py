...
class TestAPIWiring:
    ...
    WIRED_PAGES = [
        ...
        ("boards.html", {"/api/boards", "/api/collaboration/comments"}),
        ("workdocs.html", {"/api/workdocs", "/api/collaboration/comments"}),
...
class TestCollaborationEmbedding:
    """Verify collaboration features are embedded in boards and workdocs."""

    def test_boards_has_collaboration_api_calls(self):
        calls = _extract_api_calls_from_html(PROJECT_ROOT / "boards.html")
        assert any("/api/collaboration" in c for c in calls), (
            f"boards.html missing /api/collaboration calls: {calls}"
        )

    def test_boards_has_comments_panel(self):
        text = (PROJECT_ROOT / "boards.html").read_text()
        assert "comments-panel" in text

    def test_boards_has_notifications_panel(self):
        text = (PROJECT_ROOT / "boards.html").read_text()
        assert "notifications-panel" in text

    def test_workdocs_has_collaboration_api_calls(self):
        calls = _extract_api_calls_from_html(PROJECT_ROOT / "workdocs.html")
        assert any("/api/collaboration" in c for c in calls), (
            f"workdocs.html missing /api/collaboration calls: {calls}"
        )

    def test_workdocs_has_comments_panel(self):
        text = (PROJECT_ROOT / "workdocs.html").read_text()
        assert "panel-comments" in text or "comment-list" in text

    def test_workdocs_has_notifications(self):
        text = (PROJECT_ROOT / "workdocs.html").read_text()
        assert "notification" in text.lower()


class TestFounderUpdateEmbedding:
    """Verify founder update features are embedded in management.html."""

    def test_management_has_founder_update_api_calls(self):
        calls = _extract_api_calls_from_html(PROJECT_ROOT / "management.html")
        assert any("/api/founder/recommendations" in c for c in calls), (
            f"management.html missing /api/founder/recommendations calls: {calls}"
        )

    def test_management_has_founder_health_call(self):
        calls = _extract_api_calls_from_html(PROJECT_ROOT / "management.html")
        assert any("/api/founder/health" in c for c in calls), (
            f"management.html missing /api/founder/health calls: {calls}"
        )

    def test_management_has_founder_history_call(self):
        calls = _extract_api_calls_from_html(PROJECT_ROOT / "management.html")
        assert any("/api/founder/history" in c for c in calls), (
            f"management.html missing /api/founder/history calls: {calls}"
        )

    def test_management_has_founder_tab(self):
        text = (PROJECT_ROOT / "management.html").read_text()
        assert "founder" in text.lower() and "view-founder" in text
