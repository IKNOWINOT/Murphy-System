...
export async function callModel(args: { profile: 'mini'|'turbo'; messages: Msg[]; json?: boolean; maxTokens?: number }) {
  ...
  const base = String(input.task || 'vallon task').slice(0, 120);
  const verbs = ['prioritize', 'budget', 'throttle', 'schedule', 'gate'];
  ...
  const result = { chain_id:`vallon_gp_${Date.now()}`, level:Math.min(3,tasks.length), tasks, confidence:0.8 };
