...
class LLMRouter:
    ...
    Note: Groq and Aristotle use separate API keys
    """
    ...
    async def route_request(self, prompt: str, task_type: str = "generative") -> str:
        ...
        if task_type in ["verification", "deterministic"] and self.aristotle_available:
    ...
    async def _call_groq(self, prompt: str) -> str:
        """Call Groq API with automatic key rotation (generative mode, temp 0.7)"""
        try:
            ...
            response = client.chat.completions.create(
                ...
                temperature=0.7,  # Higher temperature for creative/generative tasks
                max_tokens=2000
            )
    ...
    async def _call_aristotle(self, prompt: str) -> str:
        ...
        try:
            ...
            # Use separate Aristotle client with low temperature for deterministic results
...
class MurphySystemRuntime:
    ...
    async def generate_prompts_from_document(self, doc_id: str) -> Dict[str, str]:
        ...
        for domain_name in domain_names:
            ...
            Domain: {domain_name}
            Purpose: {domain_obj.purpose if domain_obj else 'Generate requirements'}
    ...
    def generate_domain_gates(self, domain_name: str) -> List[Dict[str, Any]]:
        ...
        if domain_obj and domain_obj.gates:
            for gate_name in domain_obj.gates:
                gate = {
                    ...
                    "domain_object": domain_obj.to_dict() if domain_obj else None,
                    "severity": 0.9,  # Default severity
                    "status": "active",
                    "created_at": datetime.now().isoformat()
                }
                gates.append(gate)
            ...
            # Fallback to default gate templates
            gate_templates = {
                "engineering": [
                    {"name": "Technical Feasibility Gate", "severity": 0.9},
                    {"name": "Architecture Review Gate", "severity": 0.95},
                    {"name": "Security Review Gate", "severity": 1.0}
                ],
                "financial": [
                    {"name": "Budget Approval Gate", "severity": 0.8},
                    {"name": "ROI Validation Gate", "severity": 0.7},
                    {"name": "Financial Viability Gate", "severity": 0.85}
                ],
                "legal": [
                    {"name": "Legal Review Gate", "severity": 1.0},
                    {"name": "Compliance Validation Gate", "severity": 0.95},
                    {"name": "Contract Review Gate", "severity": 0.9}
                ],
                "operations": [
                    {"name": "Operational Readiness Gate", "severity": 0.85},
                    {"name": "Quality Assurance Gate", "severity": 0.9},
                    {"name": "Process Validation Gate", "severity": 0.8}
                ]
            }
            
            templates = gate_templates.get(domain_name, [{"name": f"{domain_name.title()} Validation Gate", "severity": 0.8}])
...
def magnify_document(doc_id):
    ...
    data = request.json
    domain_name = data.get('domain_name', data.get('domain', 'general'))  # Support both for backward compatibility
...
def solidify_document(doc_id):
    ...
    # Generate prompts (synchronous for now)
    import asyncio
    try:
        loop = asyncio.get_event_loop()
        prompts = loop.run_until_complete(runtime.generate_prompts_from_document(doc_id))
    except RuntimeError:
        # No event loop, create one
        prompts = asyncio.run(runtime.generate_prompts_from_document(doc_id))
    ...
    # Generate gates
    gates = []
    for domain_name in ["engineering", "financial", "legal"]:
        gates.extend(runtime.generate_domain_gates(domain_name))
...
if __name__ == '__main__':
    ...
    print(f"Groq (Generative) Available: {len(groq_clients) > 0}")
    print(f"Aristotle (Deterministic) Available: {aristotle_client is not None}")
