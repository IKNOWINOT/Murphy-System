// src/clockwork/bots/clarifier_bot/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['clarify','clarifier','spec','requirements','ambiguous','unknowns','blocker','acceptance criteria','assumptions','missing'];
  let score = 0; for (const k of keywords) if (task.includes(k)) score += 0.18;
  if (input.attachments?.length) score += 0.08;
  if (task.split(/\s+/).length <= 18) score += 0.04;
  const confidence = Math.min(1, Math.max(0, score));

  const est_cost_usd = 0.002;
  const must_have_inputs: string[] = []; // messaging access optional
  ...
  return RollcallOutputSchema.parse({
    ...
    gp_candidate,
    archetype: 'veritas' // clarifying leans verification
  });
}
export default { ping };
