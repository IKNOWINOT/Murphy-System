// src/clockwork/bots/code_translator_bot/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['translate','refactor','fix','bug','lint','format','tests','unit test','explain','document','docstring','patch','diff'];
  let score = 0; for (const k of keywords) if (t.includes(k)) score += 0.16;
  if (input.attachments?.length) score += 0.08;
  if (t.split(/\s+/).length <= 18) score += 0.04;
  const confidence = Math.min(1, Math.max(0, score));

  const est_cost_usd = 0.003;
  const must_have_inputs: string[] = []; // optional
  ...
  try { const key = { task_type: t.slice(0,128), params_preview: JSON.stringify(input.params||{}).slice(0,512) }; const gp = await selectPath(ctx.db as any, key as any, 1); if (gp && gp.confidence>0.75) gp_candidate = { id: gp.id, confidence: gp.confidence }; } catch(e) { ctx.logger?.warn?.('selectPath failed', e); }
  ...
  return RollcallOutputSchema.parse({
    ...
    gp_candidate,
    archetype: 'kiren' // coding+translation leans execution
  });
}
export default { ping };
