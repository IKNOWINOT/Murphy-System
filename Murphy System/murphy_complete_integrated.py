...
# Event Log Storage
event_logs = []
MAX_LOGS = 1000  # Keep last 1000 events

def log_event(event_type, command, request_data, response_data, error=None):
    """Store event log for later retrieval"""
...
def llm_generate():
    ...
    try:
        ...
        # Log the event
        event_id = log_event('llm_generate', prompt, data, result)
        result['event_id'] = event_id
        
        return jsonify(result)
    except Exception as e:
        error_result = {'error': str(e)}
        event_id = log_event('llm_generate', prompt, data, None, str(e))
        error_result['event_id'] = event_id
...
def get_all_logs():
    ...
    return jsonify({'logs': event_logs[-limit:]})
...
def librarian_ask():
    ...
    try:
        ...
        # Log the event
        event_id = log_event('librarian_ask', query, data, result)
        result['event_id'] = event_id
        ...
        error_result = {'error': str(e)}
        event_id = log_event('librarian_ask', query, data, None, str(e))
        error_result['event_id'] = event_id
        return jsonify(error_result), 500
