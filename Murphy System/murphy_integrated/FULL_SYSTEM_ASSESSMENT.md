...
141. Extended malformed `compute_request` fallback parity coverage to include `math_required.prompt` while preserving deterministic route-source behavior and no-session-side-effect guards for non-expression cases; latest targeted compute-plane validation result is **88 passed, 0 failed** (`tests/test_compute_plane_validation.py`, warnings remain pre-existing deprecations).
