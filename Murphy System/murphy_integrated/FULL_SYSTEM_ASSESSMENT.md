...
- **Compute plane validation path:** deterministic compute requests can now be validated through the runtime for structured checks, including `deterministic_request`, `deterministic_required` + `compute_expression`, `confidence_required` + `confidence_expression`, and math task-type deterministic routing.
...
2. **Compute plane + stability controllers**  
   Deterministic reasoning now supports tagged task routing (`deterministic_request`, `deterministic_required`, confidence-engine deterministic tags, and math task-type routing), but broader policy-driven compute routing still needs full rollout.
...
- **Deterministic + LLM routing:** compute plane and LLM orchestration must both be wired with clear task routing rules; deterministic-tag aliases now route to compute validation in `execute_task`, including confidence-engine and math deterministic lanes.
...
| Deterministic + LLM routing | 41.74% | Routing heuristics exist; deterministic task aliases now route through compute validation in `execute_task` (including confidence-engine and math deterministic lanes), with focused tests for alias + required-routing payloads |
...
- Deterministic + LLM routing: **+0.36%**
...
- Deterministic + LLM routing: **41.74%**
...
- **Per-prompt micro-increment delta (this prompt, decimal precision 0.01):**
  ...
  - UI + user testing: **+0.01%**
  - Dynamic-chain tests: **+0.04%**
- Latest targeted run (`test_compute_plane_validation.py`): **6 passed, 0 failed, warnings are pre-existing third-party deprecations**
