...
def test_mfgc_adapter_execution_payload():
    runtime = load_runtime_module()
    if runtime.MFGCAdapter is None:
        pytest.skip("MFGC adapter not available in test environment")
    system = runtime.MurphySystem.create_test_instance()
    system.sessions = {}

    system.system_integrator = StubIntegrator()
    system.mfgc_adapter = runtime.MFGCAdapter(system.system_integrator)
...
def test_simulation_uses_mfgc_fallback_when_available():
    runtime = load_runtime_module()
    if runtime.MFGCAdapter is None:
        pytest.skip("MFGC adapter not available in test environment")
    system = runtime.MurphySystem.create_test_instance()
