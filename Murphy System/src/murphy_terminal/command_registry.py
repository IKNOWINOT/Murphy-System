...
MURPHY_COMMANDS: list[CommandDefinition] = [
    ...
    # ── INDUSTRIAL AUTOMATION & BAS ─────────────────────────────────────────
    CommandDefinition("bas_equipment_ingestion", "/bas ingest", "!murphy bas ingest",
        ["upload equipment data", "ingest BAS data", "load equipment CSV", "add equipment spec", "BAS data import"],
        CommandCategory.IOT,
        "Ingest equipment data (CSV/JSON/EDE) into the BAS equipment registry with ASHRAE/Grainger recommendations",
        "/bas ingest <file_or_data>", requires_args=True, min_role="operator",
        subcommands=["csv", "json", "ede", "auto"],
        examples=["!murphy bas ingest --file ahu_points.csv", "!murphy bas ingest --format ede"]),
    CommandDefinition("virtual_controller", "/bas controller", "!murphy bas controller",
        ["virtual controller", "simulate controller", "controller wiring", "verify wiring", "BAS controller"],
        CommandCategory.IOT,
        "Manage virtual controllers: populate from specs, verify wiring, read/write points",
        "/bas controller <equipment_id>", requires_args=True, min_role="operator",
        subcommands=["create", "verify", "read", "write", "status", "points"],
        examples=["!murphy bas controller create --spec ahu_01", "!murphy bas controller verify ahu_01"]),
    CommandDefinition("industry_automation_wizard", "/wizard industry", "!murphy wizard industry",
        ["industry wizard", "automation wizard", "setup industry automation", "configure automation type", "industry setup"],
        CommandCategory.AUTOMATION,
        "Conversational wizard for any industry automation type (10 industries, 66+ types)",
        "/wizard industry [--industry <type>] [--automation <type>]", min_role="developer",
        subcommands=["start", "answer", "status", "spec", "list"],
        examples=["!murphy wizard industry --industry manufacturing", "!murphy wizard industry list"]),
    CommandDefinition("org_chart_generator", "/org virtual", "!murphy org virtual",
        ["virtual org chart", "generate org chart", "virtual employees", "build virtual team", "shadow agents"],
        CommandCategory.ORG,
        "Generate a virtual org chart with shadow agents for companies without real employees",
        "/org virtual [--company <name>] [--industry <type>]", min_role="operator",
        subcommands=["create", "answer", "generate", "hire", "positions", "open"],
        examples=["!murphy org virtual create", "!murphy org virtual hire --position cto"]),
    CommandDefinition("production_deliverable_wizard", "/wizard deliverable", "!murphy wizard deliverable",
        ["deliverable wizard", "production wizard", "create deliverable", "what do you want to produce"],
        CommandCategory.AUTOMATION,
        "Conversational wizard for any production deliverable: report/dashboard/workflow/document/API/analysis",
        "/wizard deliverable [--type <deliverable_type>]", min_role="developer",
        subcommands=["start", "answer", "spec", "types"],
        examples=["!murphy wizard deliverable --type report", "!murphy wizard deliverable --type dashboard"]),
    CommandDefinition("universal_ingestion_framework", "/ingest auto", "!murphy ingest auto",
        ["auto ingest", "detect and ingest", "universal ingest", "smart data import", "protocol ingest"],
        CommandCategory.DATA,
        "Auto-detect format and ingest any automation system data: BACnet EDE, Modbus, OPC-UA, CSV, JSON",
        "/ingest auto <data>", requires_args=True, min_role="operator",
        subcommands=["auto", "bacnet", "modbus", "opcua", "csv", "json", "mqtt", "adapters"],
        examples=["!murphy ingest auto --file points.ede", "!murphy ingest auto --format modbus"]),
    CommandDefinition("climate_resilience_engine", "/climate zone", "!murphy climate zone",
        ["climate zone", "resilience factors", "design conditions", "location climate", "ASHRAE climate zone"],
        CommandCategory.INTEGRATIONS,
        "Look up ASHRAE 169-2021 climate zone and resilience factors for any location worldwide",
        "/climate zone <location>", requires_args=True, min_role="viewer",
        subcommands=["zone", "resilience", "recommendations", "targets", "sizing"],
        examples=["!murphy climate zone Miami", "!murphy climate zone 'Chicago, IL' --type recommendations"]),
    CommandDefinition("energy_efficiency_framework", "/energy audit", "!murphy energy audit",
        ["energy audit", "CEM analysis", "ECM recommendations", "energy efficiency", "utility analysis", "energy conservation"],
        CommandCategory.INTEGRATIONS,
        "CEM-level energy audit, ECM recommendations, utility analysis, and ROI calculations",
        "/energy audit [--level I|II|III] [--facility-type <type>]", min_role="operator",
        subcommands=["analyze", "recommend", "roi", "audit", "benchmark", "magnify", "simplify", "solidify"],
        examples=["!murphy energy audit --level II", "!murphy energy audit recommend --facility office"]),
    CommandDefinition("synthetic_interview_engine", "/interview start", "!murphy interview start",
        ["synthetic interview", "21 questions", "knowledge elicitation", "system interview", "technical interview"],
        CommandCategory.INTELLIGENCE,
        "21-question structured knowledge elicitation with LLM inference and multi-reading-level adaptation",
        "/interview start [--domain <domain>] [--level <reading_level>]", min_role="developer",
        subcommands=["start", "next", "answer", "status", "model", "export"],
        examples=["!murphy interview start --domain BAS", "!murphy interview start --domain healthcare"]),
    CommandDefinition("system_configuration_engine", "/configure system", "!murphy configure system",
        ["configure system", "system config", "control strategy", "detect system type", "setup controls"],
        CommandCategory.AUTOMATION,
        "Detect system type, select optimal control strategy, apply MSS configuration modes",
        "/configure system <description>", requires_args=True, min_role="operator",
        subcommands=["detect", "strategies", "configure", "magnify", "simplify", "solidify"],
        examples=["!murphy configure system 'air handling unit with VFD'", "!murphy configure system detect"]),
    CommandDefinition("as_built_generator", "/asbuilt generate", "!murphy asbuilt generate",
        ["as-built", "as built drawings", "point schedule", "control diagram", "generate schematic"],
        CommandCategory.IOT,
        "Generate as-built control diagrams, point schedules, and schematics from equipment specs",
        "/asbuilt generate <equipment_id>", requires_args=True, min_role="operator",
        subcommands=["generate", "schedule", "schematic", "merge", "check", "export"],
        examples=["!murphy asbuilt generate ahu_01", "!murphy asbuilt schedule --controller ahu_01"]),
    CommandDefinition("pro_con_decision_engine", "/decide", "!murphy decide",
        ["decide", "pro con analysis", "decision engine", "evaluate options", "best option", "weigh options"],
        CommandCategory.INTELLIGENCE,
        "Weigh pros/cons, apply hard safety/compliance constraints, return optimal decision",
        "/decide <question>", requires_args=True, min_role="viewer",
        subcommands=["evaluate", "ecms", "equipment", "strategies", "explain"],
        examples=["!murphy decide 'which chiller strategy is best'", "!murphy decide equipment"]),
]
