...
class ExportPipeline:
    ...
    def _markdown_to_pdf(self, markdown: str, brand: BrandProfile) -> str:
        """Convert Markdown to base64-encoded PDF, using reportlab if available."""
        try:
            import base64
            ...
            font_name = brand.font_body if brand.font_body in ("Helvetica", "Times-Roman", "Courier") else "Helvetica"
            font_size = 11
            c.setFont(font_name, font_size)
            y = 750
            for line in self._markdown_to_text(markdown).split("\n"):
                if y < 60:
                    c.showPage()
                    c.setFont(font_name, font_size)
                    y = 750
            ...
            plain = self._markdown_to_text(markdown)
            return base64.b64encode(plain.encode("utf-8")).decode("ascii")

    def _markdown_to_word(self, markdown: str, brand: BrandProfile) -> str:
        """Convert Markdown to base64-encoded DOCX, using python-docx if available."""
        try:
            import base64
            from io import BytesIO
