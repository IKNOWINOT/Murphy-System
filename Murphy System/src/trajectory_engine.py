...
_MIN_BODY_RETENTION_RATIO         = 0.5   # candle body must retain ≥50 % of prev body to extend green run
_DEFAULT_MAX_ENTRY_PULLBACK_PCT   = 0.015 # maximum 1.5 % pullback from current price for entry
...
class TrajectoryEngine:
    ...
    def _count_green_run(self, candles: List[Candle], reasoning: List[str]) -> int:
        ...
        for c in reversed(candles):
            if not c.is_green:
                break
            if prev_body is not None and c.body < prev_body * _MIN_BODY_RETENTION_RATIO:
    ...
    def _optimal_entry(
        ...
        if len(closes) >= 5:
            ...
            std = current * 0.01
        pullback_pct = min(_DEFAULT_MAX_ENTRY_PULLBACK_PCT, std / current)
