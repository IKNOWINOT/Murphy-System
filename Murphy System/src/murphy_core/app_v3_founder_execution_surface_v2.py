...
class CoreV3FounderExecutionSurfaceV2Services:
...
def create_app() -> FastAPI:
    services = CoreV3FounderExecutionSurfaceV2Services()
    app = FastAPI(
        title=services.config.app_name,
        description="Murphy System founder execution surface v2 with aligned runtime truth and capability-aware gating",
    ...
    async def health() -> JSONResponse:
        return JSONResponse(
            {
                "status": "healthy",
                "service": "murphy_core_v3_founder_execution_surface_v2",
