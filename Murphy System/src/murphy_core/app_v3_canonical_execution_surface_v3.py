...
from .operator_runtime_surface_v5 import OperatorRuntimeSurfaceV5
...
class CoreV3CanonicalExecutionSurfaceV3Services:
    def __init__(self, config: CoreConfig | None = None) -> None:
        ...
        self.runtime_surface = OperatorRuntimeSurfaceV5(self.operator_status)
...
def create_app() -> FastAPI:
    services = CoreV3CanonicalExecutionSurfaceV3Services()
    app = FastAPI(
        title=services.config.app_name,
        description="Murphy System canonical execution surface v3 with founder visibility overlay and runtime truth v5",
    ...
    async def health() -> JSONResponse:
        return JSONResponse(
            {
                "status": "healthy",
                "service": "murphy_core_v3_canonical_execution_surface_v3",
