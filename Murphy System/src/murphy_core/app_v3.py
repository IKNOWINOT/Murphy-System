...
from .operator_status import OperatorStatusService
...
class CoreV3Services:
    def __init__(self, config: CoreConfig | None = None) -> None:
        ...
        self.operator_status = OperatorStatusService(
            self.config,
            self.registry,
            self.providers,
            self.gates,
...
def create_app() -> FastAPI:
    services = CoreV3Services()
    app = FastAPI(
        title=services.config.app_name,
        description="Canonical typed runtime spine for Murphy System (v3)",
    ...
    async def effective_capabilities() -> JSONResponse:
        snapshot = services.operator_status.snapshot()
