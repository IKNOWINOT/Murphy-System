...
from .operator_runtime_surface_v3 import OperatorRuntimeSurfaceV3
...
class CoreV3FounderExecutionSurfaceV3Services:
    def __init__(self, config: CoreConfig | None = None) -> None:
        ...
        self.runtime_surface = OperatorRuntimeSurfaceV3(self.operator_status)
...
def create_app() -> FastAPI:
    services = CoreV3FounderExecutionSurfaceV3Services()
    app = FastAPI(
        title=services.config.app_name,
        description="Murphy System founder execution surface v3 with aligned runtime truth, capability-aware gating, and subsystem family selection",
    ...
    async def health() -> JSONResponse:
        return JSONResponse(
            {
                "status": "healthy",
                "service": "murphy_core_v3_founder_execution_surface_v3",
