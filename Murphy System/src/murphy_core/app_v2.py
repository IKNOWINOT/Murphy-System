...
def create_app() -> FastAPI:
    services = CoreV2Services()
    app = FastAPI(
        title=services.config.app_name,
        description="Canonical typed runtime spine for Murphy System (v2)",
    ...
    async def effective_capabilities() -> JSONResponse:
        records = services.registry.list()
        return JSONResponse({
            "success": True,
            "summary": {
                "total_modules": len(records),
                "provider_preferred": services.providers.health().get("preferred_provider"),
                "gate_count": len(services.gates.health().get("gates", [])),
            },
        })
