...
class HetznerDeployPlanGenerator:
    ...
    def _full_deploy_plan(self, report: HetznerDeployProbeReport) -> SetupPlan:
        ...
        if report.docker_installed:
            steps.append(SetupStep(
                ...
                command=(
                    ...
                    f'-f "murphy_system/Dockerfile" "murphy_system/"'
                ),
                liability_note="You approved this action. Murphy executed it as instructed.",
            ))
        ...
        steps.append(SetupStep(
            ...
            risk_level=RiskLevel.LOW,
            command=(
                f'bash "murphy_system/scripts/production_readiness_check.sh" '
                f'{self.namespace}'
            ),
            liability_note="Read-only verification. No system change.",
        ))
...
jobs:
  test:
    ...
    env:
      MURPHY_ENV: test
      PYTHONPATH: "${{ github.workspace }}/Murphy System:${{ github.workspace }}/murphy_system/src"
    steps:
      - uses: actions/checkout@v4
      - uses: actions/setup-python@v5
        with:
          python-version: "3.12"
      - run: pip install -r "murphy_system/requirements_murphy_1.0.txt"
      - run: pip install pytest pytest-asyncio pytest-timeout
      - run: python -m pytest "murphy_system/tests/" --timeout=300 -x -q --ignore="murphy_system/tests/e2e"
