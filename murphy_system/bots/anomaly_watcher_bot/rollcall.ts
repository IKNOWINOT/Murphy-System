// src/clockwork/bots/anomaly_watcher_bot/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['anomaly','drift','outlier','spike','alert','error rate','latency spike','slo','sla','incident','regression'];
  let score = 0; for (const k of keywords) if (task.includes(k)) score += 0.14;
  if (input.attachments?.length) score += 0.12;
  if (task.split(/\s+/).length <= 12) score += 0.06;
  const confidence = Math.min(1, Math.max(0, score));

  const est_cost_usd = 0.002;
  const must_have_inputs: string[] = [];
  if (task.includes('alert') || task.includes('notify')) must_have_inputs.push('notification_access');
  ...
  return RollcallOutputSchema.parse({
    ...
    gp_candidate,
    archetype: 'veritas' // anomaly detection leans verification & monitoring
  });
}
export default { ping };
