...
vi.mock('../../../integrations/verification_adapter', () => ({
  verify: vi.fn(async (emails:string[]) => emails.map(e => ({ email:e, status:'valid' })))
}));

vi.mock('../../../io/web_fetch', () => ({
  fetchText: vi.fn(async (_url:string) => '<html><body>email: a@ex.com</body></html>')
}));
vi.mock('../../../io/html_to_text', () => ({ htmlToText: (h:string) => h.replace(/<[^>]+>/g,' ') }));

vi.mock('../../../integrations/mailbox_adapter', () => ({
  fetchNew: vi.fn(async (_opts:any) => ({
    nextCursor: 'cur2',
    messages: [
