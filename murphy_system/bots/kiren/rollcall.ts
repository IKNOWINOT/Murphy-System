// src/clockwork/bots/kiren/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['plan', 'design', 'create', 'draft', 'generate', 'synthesize'];
  ...
  if ('kiren'==='veritas' && (task.includes('validate')||task.includes('cite')||task.includes('compare'))) must_have_inputs.push('retriever_access');
