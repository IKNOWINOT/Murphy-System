...
export async function callModel(args: { profile: 'mini'|'turbo'; messages: Msg[]; json?: boolean; maxTokens?: number }) {
  ...
  const base = String(input.task || 'kaia task').slice(0, 120);
  const verbs = ['route', 'explain', 'summarize', 'nominate', 'handoff'];
  ...
  const result = { chain_id:`kaia_gp_${Date.now()}`, level:Math.min(3,tasks.length), tasks, confidence:0.8 };
