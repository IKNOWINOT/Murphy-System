// src/clockwork/bots/optimizer_core_bot/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['optimize','optimization','hyperparameter','bayes','random search','grid search','objective','loss','metric','threshold','maximize','minimize','constraints','search space','variables'];
  let score = 0; for (const k of keywords) if (task.includes(k)) score += 0.14;
  if (input.attachments?.length) score += 0.10;
  if (task.split(/\s+/).length <= 16) score += 0.06;
  const confidence = Math.min(1, Math.max(0, score));

  const est_cost_usd = 0.003;
  const must_have_inputs: string[] = ['optimizer_access'];
