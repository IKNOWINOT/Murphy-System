...
class TestStep3And4_ProbeAndGap:
    ...
    # ── murphy_system/ mirror ──────────────────────────────────────────
    def test_MIRROR_login_identical(self):
        """PROBE: login.html and murphy_system/login.html are byte-identical."""
        root = (REPO_ROOT / "login.html").read_text()
        mirror = (REPO_ROOT / "murphy_system" / "login.html").read_text()
        r = ProbeResult(
    ...
    def test_MIRROR_signup_identical(self):
        """PROBE: signup.html and murphy_system/signup.html are byte-identical."""
        root = (REPO_ROOT / "signup.html").read_text()
        mirror = (REPO_ROOT / "murphy_system" / "signup.html").read_text()
        r = ProbeResult(
    ...
    def test_MIRROR_landing_identical(self):
        """PROBE: murphy_landing_page.html and murphy_system/ copy are identical."""
        root = (REPO_ROOT / "murphy_landing_page.html").read_text()
        mirror = (REPO_ROOT / "murphy_system" / "murphy_landing_page.html").read_text()
        r = ProbeResult(
...
class TestStep6_VerifyGapsClosed:
    ...
    def test_verify_mirror_sync_after_fixes(self):
        ...
        pairs = [
            ("login.html", "murphy_system/login.html"),
            ("signup.html", "murphy_system/signup.html"),
            ("murphy_landing_page.html", "murphy_system/murphy_landing_page.html"),
        ]
        for a, b in pairs:
