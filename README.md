...
- **Latest targeted test result:** `tests/test_execution_wiring_integration.py` + `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **139 passed, 0 failed, 27 skipped** (warnings are pre-existing dependency deprecations); policy-block paths now reuse caller-provided `session_id` without invoking `create_session()`, including orchestrator-unavailable enforced blocks, and fallback execution now handles both missing `create_session()` payloads and `create_session()` exceptions without raising.
