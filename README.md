...
- **Competitive alignment:** activation previews and `/api/status` now include `competitive_feature_alignment` plus `competitive_feature_alignment_summary` and `module_registry_summary`; `/api/info` includes alignment + integration summaries for lightweight capability reporting.
