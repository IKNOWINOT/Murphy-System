...
- **Delivery connector configuration:** provide `delivery_connectors` in `/api/execute` to mark adapters as configured in runtime previews (connectors without a `channel` default to `unknown`; missing statuses are treated as `unconfigured`, and invalid values log warnings before defaulting).
