...
- **Latest targeted test result:** `tests/test_compute_plane_validation.py` — **11 passed, 0 failed** (warnings are pre-existing dependency deprecations); non-expression confidence/math prompts and `math_required` fallback paths now verify no unnecessary compute-session side effects.
