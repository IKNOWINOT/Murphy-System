...
- **Delivery adapter snapshot:** activation previews include document/email/chat/voice adapter readiness; summary counts reflect adapter statuses (configured, available, unconfigured), and delivery readiness passes through `needs_wiring`/`needs_coverage` when adapters or org chart coverage are incomplete.
