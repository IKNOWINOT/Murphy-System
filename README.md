...
- **Delivery adapter snapshot:** activation previews include document/email/chat/voice adapter readiness; summary counts show configured vs available adapters, and delivery readiness passes through `needs_wiring`/`needs_coverage` when adapters or org chart coverage are incomplete.
