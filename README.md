...
- **Latest targeted test result:** `tests/test_compute_plane_validation.py` — **12 passed, 0 failed** (warnings are pre-existing dependency deprecations); non-expression confidence/math prompts plus `math_required`/`confidence_required` fallback paths now verify no unnecessary compute-session side effects.
