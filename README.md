...
- **Deterministic validation:** supply `compute_request` (expression + language) to `/api/execute` for cached compute plane checks (thread-safe shared service instance); session/doc tracking and logging attach once validation succeeds, with warnings if session creation fails.
