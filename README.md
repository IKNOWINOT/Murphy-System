...
- **Latest targeted test result:** `tests/test_execution_wiring_integration.py` + `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **147 passed, 0 failed, 40 skipped** (warnings are pre-existing dependency deprecations); policy-block paths now reuse caller-provided `session_id` without invoking `create_session()`, including orchestrator-unavailable enforced blocks, fallback execution handles missing/invalid/exception `create_session()` payloads without raising while exposing explicit fallback orchestration metadata (`metadata.orchestration_mode`), fallback metadata timestamps are emitted as timezone-aware UTC values (including direct `_simulate_execution` fallback coverage without adapter dependency), string and bytes policy flags (for example `enforce_policy="false"` and `enforce_policy=b"false"`) are normalized before enforcement decisions, `require_orchestrator_online` string flags now coerce consistently (`"true"` blocks when unavailable, `"false"` allows fallback), zero-like fallback session IDs from `create_session()` payloads are preserved (`0` → `"0"`), and container/bytes/non-finite numeric fallback session IDs are rejected and normalized to `None`.
