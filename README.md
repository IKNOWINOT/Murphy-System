...
- **Completion snapshot surface test:** `tests/test_completion_snapshot_surface.py` validates completion snapshot parity across activation preview, `/api/status`, and `/api/info`.
  It covers threshold metadata plus runtime execution profile parity for mode, governance, control-plane separation, shadow-agent/account policies, user-base governance, and employee-contract responsibility/change/accountability/escalation policy derivation checks.
