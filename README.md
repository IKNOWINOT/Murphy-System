...
- **Latest targeted test result:** `tests/test_execution_wiring_integration.py` + `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **161 passed, 0 failed, 44 skipped** (warnings are pre-existing dependency deprecations); policy-block paths now reuse caller-provided `session_id` without invoking `create_session()`, including orchestrator-unavailable enforced blocks, fallback execution handles missing/invalid/exception `create_session()` payloads without raising while exposing explicit fallback orchestration metadata (`metadata.orchestration_mode`), fallback metadata timestamps are emitted as timezone-aware UTC values (including direct `_simulate_execution` fallback coverage without adapter dependency), MFGC fallback payload timestamps are now timezone-aware UTC (`mfgc_execution.timestamp`), string and bytes policy flags (for example `enforce_policy="false"` and `enforce_policy=b"false"`) are normalized before enforcement decisions, malformed container policy payloads for `require_orchestrator_online` now fall back to the default behavior (instead of truthy coercion), non-finite and complex numeric policy payloads (for example `require_orchestrator_online=NaN`, `Decimal(\"Infinity\")`, or `(1+0j)`) now default safely instead of coercing truthy, uncoercible policy-flag objects (including generic `Exception`-raising `__bool__` implementations) now default safely instead of crashing policy evaluation, zero-like fallback session IDs from `create_session()` payloads are preserved (`0` → `"0"`), container/bytes/non-finite numeric fallback session IDs (including `frozenset` and `Decimal(\"NaN\")`) are rejected and normalized to `None`, unstringifiable fallback session-id payload objects now degrade safely to `None` instead of raising during normalization, and compute-validation session creation now degrades safely when `create_session()` returns invalid non-dictionary/session-id payloads or raises while valid payload IDs are now auto-registered in the runtime session registry before document mapping with timezone-aware UTC `created_at` values.
