...
- **Latest targeted test result:** `tests/test_execution_wiring_integration.py` + `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **138 passed, 0 failed, 25 skipped** (warnings are pre-existing dependency deprecations); in addition to deterministic-routing/session-binding behavior, runtime hardening now enforces policy-driven blocking when the orchestrator is unavailable (no fallback payload when `enforce_policy=True`, with canonical blocked `reason` in both policy-block paths), deterministic-required route affinity + gate/swarm execution-mode metadata exposure in execution responses, and compute-service hardening for pending-request deduplication, canonical request-signature hashing, request-id collision cache-safety, timeout handling (`TIMEOUT`), post-shutdown replay guards, stale-worker overwrite prevention, worker-start rollback safety, and defensive result-copy semantics in `ComputeService.get_result`.
