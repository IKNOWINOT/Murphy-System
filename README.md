...
- **Latest targeted test result:** `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **119 passed, 0 failed, 17 skipped** (warnings are pre-existing dependency deprecations); in addition to deterministic-routing/session-binding behavior, compute-service hardening now includes pending-request deduplication, canonical request-signature hashing for nested metadata/assumption payloads, request-id collision cache-safety, explicit timeout handling (`TIMEOUT`) for long-running compute executions, immediate post-shutdown request rejection without background worker spawn, no-session allocation on compute validation error paths, preservation of explicitly supplied (unknown) session IDs during compute validation binding, safe normalization of string `compute_request` payloads, normalization of non-dict request containers (e.g., `metadata=None`), runtime guarding for mutated/unsupported language values, and whitespace-only dict expressions treated as missing compute expressions.
