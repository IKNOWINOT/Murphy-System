...
- **Latest targeted test result:** `tests/test_execution_wiring_integration.py` + `tests/test_compute_plane.py` + `tests/test_compute_plane_validation.py` — **134 passed, 0 failed, 25 skipped** (warnings are pre-existing dependency deprecations); in addition to deterministic-routing/session-binding behavior, runtime hardening now enforces policy-driven blocking when the orchestrator is unavailable (no fallback payload when `enforce_policy=True`, with canonical blocked `reason` in both policy-block paths), while compute-service hardening includes pending-request deduplication, canonical request-signature hashing for nested metadata/assumption payloads, request-id collision cache-safety with non-mutating caller request objects, explicit timeout handling (`TIMEOUT`) for long-running compute executions, immediate post-shutdown request rejection without background worker spawn, preservation of shutdown failure state for in-flight requests (workers no longer overwrite shutdown `FAIL` cache entries), preservation of existing successful cached results on same-ID post-shutdown resubmission while safely suffixing different-signature post-shutdown collisions, post-shutdown same-signature replay reuse for existing cached `FAIL` entries (no runaway suffixed IDs), no-session allocation on compute validation error paths, preservation of explicitly supplied (unknown) session IDs during compute validation binding, safe normalization of string `compute_request` payloads, normalization of non-dict request containers (e.g., `metadata=None`), runtime guarding for mutated/unsupported language values, synchronous unsupported-language preflight handling in `submit_request` (no worker thread spawn), whitespace/case normalization for supported-language variants during submit preflight, whitespace-only dict expressions treated as missing compute expressions, whitespace-only `ComputeRequest.expression` preflight rejection without worker spawn, request-id normalization for both whitespace-only IDs (generated fallback) and non-empty surrounding whitespace (trimmed IDs), worker-start failure rollback handling (no pending-state leak and synchronous `FAIL` cache result), normalization of whitespace/non-string `session_id` values before runtime orchestration fallback handling (including boolean session IDs treated as missing), and defensive result-copying in `ComputeService.get_result` to prevent caller-side mutation of cached result state.
