...
- **Two-phase orchestrator test:** `tests/test_two_phase_orchestrator_execution.py` validates the legacy phase1/phase2 create/run automation fallback path (domain defaults to task type when not provided) and uses a dedicated session ID separate from the automation ID.
