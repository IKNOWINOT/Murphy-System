...
| **Weighted overall** | **~98%** | Branch is materially further along in runtime truth, canonical execution, visibility, bootability, endpoint-level execution enforcement, explicit fallback handling, fallback boundaries, endpoint outcome signaling, trace outcome signaling, operator-facing outcome summaries, founder-overlay outcome summaries, and recovery trace alignment than mainline README baseline |
