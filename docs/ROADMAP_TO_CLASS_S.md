...
| 15 | Performance baselines (pytest-benchmark + locust/k6) | 🟡 | `tests/benchmarks/` exists with statistical (`test_benchmark_statistical.py`), API-throughput, integration-speed, and locust suites (~907 LOC). **Hot-path coverage:** governance kernel ✅ (`test_gate_evaluation_throughput`, target >50 000 ops/s), control plane ✅ (`test_control_plane_creation_throughput`, target >1 000 ops/s), platform connector framework ✅ (`test_platform_connector_framework_throughput`, target >200 ops/s), LLM provider routing ✅ (`test_llm_routing_dispatch_throughput`, target >5 000 ops/s — measures dispatch overhead on the all-backends-unavailable path; observed ~41 700 ops/s on a CI runner). **Remaining:** RAG retrieval and HITL queue dispatch benchmarks; baseline JSON committed and a `--benchmark-compare-fail=mean:10%` CI gate. |
