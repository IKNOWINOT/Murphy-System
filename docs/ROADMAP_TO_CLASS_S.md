...
| # | Item | Status | Notes |
|---|---|---|---|
| 6 | OpenTelemetry tracing + structured JSON logs + request-id middleware | 🟡 | Structured JSON logs (`src/logging_config.py`) and request-ID middleware (`src/request_context.py:RequestIDMiddleware`) are already wired in `src/runtime/app.py` (lines 8988, 15767). OTel tracing scaffold added: `src/runtime/tracing.py` provides a no-op-by-default `configure_tracing()` opt-in via `MURPHY_OTEL_ENABLED`. Wiring it into `app.py` and adding the OTel SDK to production requirements is the remaining sub-item. |
