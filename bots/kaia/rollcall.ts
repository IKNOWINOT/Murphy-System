// src/clockwork/bots/kaia/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['route', 'summarize', 'explain', 'nominate', 'handoff', 'compose'];
  ...
  if ('kaia'==='veritas' && (task.includes('validate')||task.includes('cite')||task.includes('compare'))) must_have_inputs.push('retriever_access');
