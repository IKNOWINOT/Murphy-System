// model proxy (local shim) — veritas
...
export async function callModel(args: { profile: 'mini'|'turbo'; messages: Msg[]; json?: boolean; maxTokens?: number }) {
  ...
  const base = String(input.task || 'veritas task').slice(0, 120);
  const verbs = ['validate', 'cite', 'check', 'compare', 'audit'];
  ...
  const result = { chain_id:`veritas_gp_${Date.now()}`, level:Math.min(3,tasks.length), tasks, confidence:0.8 };
