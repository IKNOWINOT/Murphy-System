// src/clockwork/bots/veritas/rollcall.ts
import { z } from 'zod';
...
export async function ping(raw: unknown, ctx: { db?: any; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  const keywords = ['validate', 'fact', 'cite', 'check', 'compare', 'audit'];
  ...
  if ('veritas'==='veritas' && (task.includes('validate')||task.includes('cite')||task.includes('compare'))) must_have_inputs.push('retriever_access');
