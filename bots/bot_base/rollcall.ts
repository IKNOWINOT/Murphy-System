// src/clockwork/bots/bot_base/rollcall.ts
// TEMPLATE rollcall (no LLM). Adjust KEYWORDS and DEFAULT_ARCHETYPE for your bot.
import { z } from 'zod';
import { selectPath } from './internal/shim_golden_paths';

const DEFAULT_ARCHETYPE = 'kiren'; // change to 'veritas' or 'vallon' as needed
...
const KEYWORDS = ['plan','create','validate','compare','summarize','transform']; // edit

export async function ping(raw: unknown, ctx: { db?: any; archetype?: string; logger?: any } = {}): Promise<RollcallOutput> {
  ...
  let score = 0; for (const k of KEYWORDS) if (task.includes(k)) score += 0.15;
  ...
  return RollcallOutputSchema.parse({
    ...
    gp_candidate,
    archetype: ctx.archetype || DEFAULT_ARCHETYPE
  });
}
export default { ping };
