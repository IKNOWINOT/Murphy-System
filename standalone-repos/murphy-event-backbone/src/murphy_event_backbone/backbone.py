"""
murphy-event-backbone — Durable in-process event bus with pub/sub,
circuit breakers, dead letter queue, retry, and backpressure detection.
...
class EventBackbone:
    ...
    Event types are plain strings — define your own domain vocabulary:

        bb = EventBackbone()
        bb.subscribe("order.created", handle_order)
        bb.publish("order.created", {"order_id": 123})
        bb.process_pending()
    """
    ...
    def __init__(
        ...
        # Queues keyed by event_type string
        self._queues: Dict[str, List[Event]] = defaultdict(list)
        # Subscriptions keyed by event_type string
    ...
    def publish(
        ...
        with self._lock:
            ...
            logger.debug("Published event %s of type %s", event_id, event_type)
            self._persist_state()
        return event_id
    ...
    def subscribe(
        ...
        with self._lock:
            ...
            self._circuit_breakers[subscription_id] = CircuitBreaker(
                failure_threshold=self._cb_threshold,
                recovery_timeout=self._cb_timeout,
            )
            logger.debug(
                "Subscription %s registered for %s", subscription_id, event_type
            )
    ...
    def _check_backpressure(self) -> None:
        """Emit a warning and a system.backpressure event if queue depth is too high."""
        with self._lock:
        ...
        try:
            with self._lock:
                self._seen_event_ids.add(event_id)
                self._queues["system.backpressure"].append(alert)
                self._events_published += 1
        except Exception:
            logger.exception("Failed to publish backpressure event")
    ...
    def _process_queue(self, event_type: str) -> int:
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            for et_value, events in state.get("queues", {}).items():
                self._queues[et_value] = [Event.from_dict(e) for e in events]
