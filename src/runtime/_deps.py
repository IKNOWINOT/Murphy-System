...
try:
    ...
    logger.warning("SelfFixLoop not available: %s", exc)
    SelfFixLoop = None

try:
    from src.scheduler import MurphyScheduler
except ImportError as exc:
    logger.warning("MurphyScheduler not available: %s", exc)
    MurphyScheduler = None

try:
    from src.autonomous_repair_system import AutonomousRepairSystem
except ImportError as exc:
    logger.warning("AutonomousRepairSystem not available: %s", exc)
...
try:
    ...
    logger.warning("Wingman system not available: %s", exc)
    WingmanSystem = None

try:
    from src.api_capability_builder import WingmanApiGapChecker
except ImportError as exc:
    logger.warning("API capability builder not available: %s", exc)
