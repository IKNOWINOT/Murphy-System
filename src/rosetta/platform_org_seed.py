...
_PLATFORM_ROSTER: List[EmployeeContract] = [
    EmployeeContract(
        ...
        reports_to=None,
        direct_reports=["cto", "compliance", "sre"],
    ),
    EmployeeContract(
        agent_type=AgentType.AUTOMATION,
        role_title="cto",
...
def seed_platform_org(manager: RosettaManager) -> List[str]:
    """Idempotently seed the Murphy-platform org into *manager*.

    Returns the list of platform agent IDs that exist after seeding
    (always the canonical four).  Raises if the manager refuses a
    write — failures are loud, not silent.
    """
    if manager is None:
        raise ValueError("seed_platform_org requires a non-None RosettaManager")

    _validate_tree(_PLATFORM_ROSTER)

    seeded_ids: List[str] = []
    for contract in _PLATFORM_ROSTER:
...
def get_platform_roster() -> List[EmployeeContract]:
