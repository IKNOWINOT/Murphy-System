...
def _classify_valence(work_summary: str, evidence: Dict[str, Any],
    ...
    """
    Template classifier. Returns (valence, confidence, reaction_seed).
    Returns reaction_seed for optional LLM enrichment.
    """
    ok = evidence.get("ok") if isinstance(evidence, dict) else None
    delta = evidence.get("delta") if isinstance(evidence, dict) else None
    gap = evidence.get("gap") if isinstance(evidence, dict) else None
    ...
    if ok is False:
        if expected and expected.get("ok") is False:
            return ("expected", 0.8,
                    "Got the failure I predicted. Reason: {}".format(reason or "n/a"))
        return ("off", 0.85,
                "Failed unexpectedly. Reason: {}".format(reason or "n/a"))
    ...
    # Gap-shaped (reality vs ledger)
    if isinstance(gap, (int, float)):
        if abs(gap) < 0.01:
            return ("expected", 0.9,
                    "Reality matched ledger exactly. Clean.")
        return ("huh", 0.7,
...
def _try_llm_enrich(agent_id: str, work_summary: str, valence: str,
    ...
    """
    Optional LLM voice enrichment. Returns None if LLM unavailable.
    Template scaffold prevents theater: must reference evidence values.
    Best-effort — if anything fails, returns None and caller uses seed.
    """
    try:
        import sys
        if "/opt/Murphy-System" not in sys.path:
            sys.path.insert(0, "/opt/Murphy-System")
        # Probe for a stable LLM entry point. Several candidates exist;
        # if none of them work, fall back to seed.
        try:
            from src.llm_provider import MurphyLLMProvider
            llm = MurphyLLMProvider()
            prompt = (
                "You are agent '{}'. You just produced output: '{}'. "
                "Valence: {}. Anchor (template seed): '{}'. "
                "Evidence keys: {}. "
                "Write a 1-2 sentence first-person reaction. "
                "MUST reference at least one concrete value from evidence. "
                "Do NOT invent facts. Be honest if unsure."
            ).format(agent_id, work_summary[:120], valence, seed,
                     list(evidence.keys())[:6] if isinstance(evidence, dict) else [])
            out = llm.generate(prompt, max_tokens=120, temperature=0.4)
            if out and isinstance(out, str) and 10 < len(out) < 600:
                return out.strip()
        except Exception:
            pass
    except Exception:
